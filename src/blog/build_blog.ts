...
const buildBlog = async (): Promise<void> => {
  ...
  const memposts = await Promise.all(
      markdowns.map((mdPath): null | Promise<Mempost> => {
    if (path.extname(mdPath) !== '.md' || path.basename(mdPath) === 'index.md') {
      return null;
    }
    const md = fs.readFileSync(path.join(postsDir, mdPath)).toString('utf8');
